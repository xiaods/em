...
export const exportContext = (context, format = 'html', indent = 0) => {
  ...
  const exportedChildren = children.length > 0
    ? `${childrenPrefix}\n${children.map(child => '  ' + exportContext(context.concat(child.key), format, indent + (format === 'html' ? (indent === 0 ? 3 : 2) : 1))).join('\n')}${childrenPostfix}${indent === 0 ? tab0 : tab1}`
    : ''
  const text = `${tab0}${linePrefix}${signifier(context)}${exportedChildren ? (tab1) : ''}${exportedChildren}${linePostfix}`
  return indent === 0 && format === 'html' ? `<ul>\n  ${text}\n</ul>` : text
}
